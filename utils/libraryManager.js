...
// 同步示例曲谱（在应用启动时调用）
// 此函数会检查版本号并自动添加/更新示例曲谱，同时保留用户数据
function syncSampleData() {
  try {
    ...
    console.log('[libraryManager] 同步示例曲谱，从版本', currentVersion, '升级到', SAMPLE_DATA_VERSION);
    ...
    sampleDef.files.forEach(sampleFile => {
      const existingIndex = data.files.findIndex(f => f.id === sampleFile.id);
      
      if (existingIndex >= 0) {
        // 更新现有文件（保留用户可能修改的部分，如 starred）
        ...
        console.log('[libraryManager] 更新示例曲谱:', sampleFile.file_name);
      } else {
        // 添加新文件
        data.files.push({
          ...sampleFile,
        ...
        console.log('[libraryManager] 添加新示例曲谱:', sampleFile.file_name);
      }
    });
    
    // 清理已删除的示例曲谱（可选：如果你从定义中移除了某个曲谱）
    // 这里我们保留用户可能已经修改过的示例曲谱，不自动删除
    
    saveStorageData(data);
    wx.setStorageSync(SAMPLE_VERSION_KEY, SAMPLE_DATA_VERSION);
    
    console.log('[libraryManager] 示例曲谱同步完成，新版本:', SAMPLE_DATA_VERSION);
    return true;
  } catch (e) {
